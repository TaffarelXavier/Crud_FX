...
<html lang="pt-br">
    <head>
        ...
        <div class="container">
            <div class="hero-unit">
                <br/><h1>TabelaTeste</h1>
                <div class="subheader"> 
                <hr/>
                <a href="#modal_tabelaTeste" role="button" class="btn btn-success" data-toggle="modal">Adicionar Novo</a>
                    <hr/>
        <!-- Modal -->
<div id="modal_tabelaTeste" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h3 id="myModalLabel">Add Novo</h3>
  </div>
  <div class="modal-body"><form action="modelos/model.tabelateste.php" id="form_tabelaTeste" method="POST">
...
<input type="hidden" name="acao" value="inserir" /></div></form><br/></div>
  <div class="modal-footer">
    ...
    <button class="btn btn-primary" form="form_tabelaTeste">Salvar mudanças</button>
  </div>
</div><table class="table"><tr><th>ID:</th>
<th>NOME:</th><th>TAB_3:</th><th>#</th></tr>
                        <?php
                        $tabelaTeste = new tabelaTeste($connection);

                        $f = $tabelaTeste->getDados(); while ($row = $f->fetch(PDO::FETCH_OBJ)) {
                            ?>
                            <tr id="linha_<?php echo $row->tab_id?>"><td><?php echo $row->tab_id?></td>
<td><?php echo $row->tab_nome?></td><td><?php echo $row->tab_tab_3?></td><td style="text-align: right;"><button class="btn btn-editar btn-success">
...
window.location.reload();
            ...
            $('#linha_'+_id).remove();
            $.post('modelos/model.tabelaTeste.php',{acao:'excluir',id:_id},function(_result){
            if(_result=='1'){
